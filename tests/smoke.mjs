...
function testTieredRefreshSource () {
  ...
  assert(source.includes('runLocalCommand'), 'latency detail should support local traceroute from Tabby to the SSH host')
  assert(source.includes('localLatencyDetailScript'), 'latency detail should have a local traceroute script')
  assert(source.includes('traceroute -n -w 1 -q 3 -m 24 "$target"'), 'local latency detail should trace from the local machine to the SSH host')
  ...
  assert(source.includes('fallback_target=__TABBY_STATUS_FALLBACK_TRACE_TARGET__'), 'latency detail should have a fallback trace target')
  assert(source.includes('is_local_trace_target()'), 'latency detail should avoid tracing the SSH server to itself')
  assert(source.includes('target="$fallback_target"'), 'latency detail should switch to fallback target when requested target is local')
  ...
  assert(source.includes('collectLatencyDetails(terminal, ssh)'), 'latency detail collection must use the dedicated long-running collector')
  ...
  assert(source.includes("this.setData(panel, 'status', '')"), 'waiting for SSH should not render a small status line under the IP row')
  ...
  assert(source.includes('--tfs-text-primary: #d7dee0'), 'main body text color must be centralized for process, disk, and meter values')
  assert(source.includes('.tfs-meter b { text-align: right'), 'meter values must align to the same right edge')
  assert(source.includes('.tfs-meter i { position: relative; height: 15px; border-radius: 999px; background: rgba(255,255,255,.12); overflow: hidden; }'), 'meter progress container should keep its existing italic text behavior')
