...
function testTieredRefreshSource () {
  ...
  assert(source.includes('printf "ldetail\\t%s\\t%s\\t%s\\t'), 'latency detail payload must include ip/latency/loss/location')
  assert(source.includes('traceroute -n -w 1 -q 1 -m 12') && source.includes('tracepath -n -m 12'), 'latency detail should show route tracing when tools are available')
  ...
  assert(source.includes('丢包'), 'latency detail headers are incomplete')
  ...
  assert(source.includes('.tfs-meter-value { display: inline-flex; align-items: center; gap: 10px; }'), 'meter percentage and detail text should stay close together')
