...
function testGithubWorkflow () {
  ...
  assert(workflow.includes('Publish latest GitHub Release'), 'workflow must update a Release-page asset on every main push')
  assert(workflow.includes("github.ref == 'refs/heads/main'"), 'latest Release publishing must run on main pushes')
  assert(workflow.includes("find release -maxdepth 1 -type f -name '*.zip' -print -quit"), 'latest Release publishing must locate the generated zip without shelling through ls')
  assert(workflow.includes('release_tag="latest"'), 'latest Release publishing must use a stable latest tag')
  assert(workflow.includes('git push origin "refs/tags/${release_tag}" --force'), 'latest Release tag must move to the current main commit')
  assert(workflow.includes('gh release upload "${release_tag}" "${zip_path}" --clobber'), 'latest Release asset must be replaced with the current zip')
  assert(workflow.includes('gh release create "${release_tag}" "${zip_path}"'), 'workflow must create the latest Release if it does not exist')
  assert(workflow.includes('Publish version GitHub Release'), 'workflow should still publish versioned releases for v* tags')
  assert(workflow.includes('softprops/action-gh-release@v2'), 'workflow must publish zip files on version tags')
  assert(workflow.includes("startsWith(github.ref, 'refs/tags/v')"), 'version Release publishing should be limited to v* tags')
