...
function testTieredRefreshSource () {
  ...
  assert(source.includes('pinnedPublicIps'), 'public IP should be pinned after first successful public lookup')
  assert(source.includes('resolveStablePublicIp'), 'merged metric state should resolve a stable public IP')
  assert(source.includes('this.pinnedPublicIps.delete(terminal)'), 'pinned public IP must be cleared when the SSH session ends or changes')
  assert(source.includes('if (sessionChanged) {\n      this.state.delete(terminal)'), 'session changes must clear old metric state so previous public IP cannot be re-pinned')
  assert(source.includes('next.publicIp = stablePublicIp'), 'empty later public-IP samples must not replace the pinned public IP')
  assert(source.includes("this.slowCollectorExecCommand(this.pinnedPublicIps.get(terminal) || '')"), 'slow collector should receive the pinned public IP')
  assert(source.includes('private slowCollectorExecCommand (pinnedPublicIp = \'\'): string'), 'slow collector command should accept a pinned public IP')
  assert(source.includes('publicIp=__TABBY_STATUS_PINNED_PUBLIC_IP__'), 'slow collector script should seed the pinned public IP before provider lookups')
  assert(source.includes('if [ -z "$publicIp" ]; then\nfor url in https://api.ipify.org'), 'public IP providers should only be queried when no public IP is pinned')
  ...
  assert(source.includes('panelCollapsedStorageKey'), 'panel collapsed state should have a dedicated storage key')
  assert(source.includes('collapsedPanelWidth = 32'), 'collapsed panel should leave a narrow visible rail with breathing room')
  assert(source.includes('data-action="panel-collapse"'), 'panel must include a boundary collapse/expand handle')
  assert(source.includes('bindPanelCollapse'), 'panel collapse handle click binding is missing')
  assert(source.includes('setPanelCollapsed'), 'panel collapse state should be applied through one layout helper')
  assert(source.includes('tabby-status-collapsed'), 'collapsed state must be represented on the layout host')
  assert(source.includes('--tfs-panel-active-width'), 'terminal and SFTP layout should use the active expanded/collapsed width')
  assert(source.includes('tfs-panel-body'), 'panel contents should be wrapped so collapsed mode can keep only the rail visible')
  assert(source.includes('tfs-collapse-toggle'), 'collapse handle styling is missing')
  assert(source.includes('aria-expanded="true"><span class="tfs-collapse-icon" aria-hidden="true"></span>'), 'collapsed expand control should render only the chevron button')
  assert(!source.includes('tfs-collapse-dot'), 'collapsed state must not render a status dot')
  assert(source.includes('.tfs-collapse-toggle:focus-visible'), 'collapse handle should have a keyboard focus state')
  assert(source.includes('box-shadow: none;'), 'collapse handle should not render as a heavy floating capsule')
  assert(source.includes('.tabby-status-layout.tabby-status-collapsed .tfs-collapse-toggle'), 'collapsed rail should restyle the expand handle')
  assert(source.includes('top: 6px;\n        left: 5px;\n        right: auto;\n        width: 22px;\n        height: 22px;'), 'collapsed expand control should be slightly inset and smaller than the rail')
  assert(source.includes('border: 1px solid transparent;\n        border-radius: 7px;\n        background: transparent;'), 'collapsed expand control should keep the same button geometry as the expanded collapse control')
  assert(source.includes('.tabby-status-layout.tabby-status-collapsed .tfs-collapse-toggle:hover {\n        color: var(--tfs-text-primary);\n        border-color: var(--tfs-border);\n        background: color-mix(in srgb, var(--tfs-text-primary) 8%, transparent);'), 'collapsed expand control hover should show the same button treatment as the collapse control')
  assert(source.includes('width: 8px;\n        height: 8px;\n        border-width: 2px;'), 'collapsed expand icon should remain a small chevron, not a status indicator')
  assert(!source.includes('scale(1.12)'), 'collapsed status dot size should not change on hover')
  assert(!source.includes('right: -14px;'), 'collapse handle should not protrude as a large external capsule')
  assert(!source.includes('height: 46px;'), 'collapse handle should not use the old tall capsule shape')
  assert(!source.includes('top: 36px;'), 'collapsed expand control should not sit below the terminal status row')
  assert(source.includes('aria-expanded'), 'collapse handle must expose expanded state to accessibility APIs')
  assert(source.includes('window.localStorage?.setItem(this.panelCollapsedStorageKey'), 'collapsed state must persist after Tabby restart')
  assert(source.includes('tfs-panel-toggling'), 'collapse/expand animation should only run during explicit toggles')
  ...
  assert(source.includes('calc(var(--tfs-panel-active-width) + var(--tfs-content-margin))'), 'terminal content margin must include the active status panel width')
  ...
  assert(source.includes('width: calc(100% - var(--tfs-panel-active-width))'), 'SFTP absolute panel width must subtract the active status panel width')
