...
- 状态面板挂载在终端外层布局容器，作为统一左侧栏显示；终端内容区和 SFTP 的绝对定位文件面板会同步让出状态栏宽度，避免命令行或文件名列被盖住。
- CPU 右侧显示百分比；内存、交换右侧同时显示百分比和容量，交换未启用时显示“未启用”。
- 进程列表的“内存”和“CPU”表头支持点击排序。
- 点击进程、网络、延迟区域可打开弹窗二级页；详情数据按 1 秒刷新。
- 二级页所有列都支持拖拽调整列宽，表格支持横向滚动。
- 进程详情展示 PID、用户、内存、CPU、命令、位置，其中 PID、用户、内存、CPU 支持排序；进程行支持右键终止进程、复制 PID、复制名称、复制命令、复制位置。
- 网络详情展示 PID、名称、监听 IP、端口、IP 数、连接数、上传、下载，所有字段支持排序；能读取 TCP byte counters 时按刷新差值显示速率，否则显示 `-`。
- 延迟详情展示到 `1.1.1.1` 的路由追踪，字段为 IP、延迟、丢包、位置。
